...
fn complex_helpers_match_expected_formulas() {
    ...
    assert_eq!(abs(x), 3.0);
    assert_eq!(abs2(x), 9.0);
    assert_eq!(real(x), 3.0);
    assert_eq!(imag(x), 0.0);
    assert_eq!(angle(x), 0.0_f64.atan2(x));
    assert_eq!(abs(z), 5.0);
    ...
    assert_eq!(real_grad, Complex64::new(2.0, 0.0));
    let imag_grad: Complex64 = imag_rrule(2.0);
    assert_eq!(imag_grad, Complex64::new(0.0, 2.0));
